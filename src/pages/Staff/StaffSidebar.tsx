...
const StaffSidebar: React.FC<StaffSidebarProps> = ({ activeItem, setActiveItem, onLogout }) => {
  const SidebarItem = ({ title, icon, isActive, onClick }: { title: string, icon: React.ReactNode, isActive: boolean, onClick: () => void }) => (
    <Button
      className={`justify-start hover:bg-default-100 ${isActive ? 'bg-primary/10 text-primary' : 'text-default-500'}`}
      variant="light"
      startContent={icon}
      onClick={onClick}
      fullWidth
            ...
            {projectExpanded && (
              <div className="ml-4 mt-2 flex flex-col gap-2">
                <SidebarItem
                  title="Sample"
                  icon={<FaChartBar />}
                  isActive={activeItem === 'project-sample'}
                  onClick={() => setActiveItem('project-sample')}
                />
                <SidebarItem
                  title="Design"
                  icon={<FaChartBar />}
                  isActive={activeItem === 'project-design'}
                  onClick={() => setActiveItem('project-design')}
                />
              </div>
            )}
          </div>
